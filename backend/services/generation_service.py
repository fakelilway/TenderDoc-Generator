...
_ANCHOR_SPECS: dict[str, tuple[tuple[str, ...], tuple[str, ...], tuple[str, ...]]] = {
    ...
    # 真实招标这张多为表格"法定代表人身份证明书"(姓名/性别/年龄/职务 + "系…的法定代表人"),
    # 正文不一定有"复印件"字样。故先按表头关键词命中表格(表格不在目录、也避开基本情况表),
    # 命不中再退正文附件行。表头关键词都选身份证明表独有、基本情况表/目录不会有的字样。
    "法定代表人身份证明": (
        ("法定代表人身份证明", "的法定代表人", "身份证明书"),
        ("法定代表人身份证复印件", "法定代表人身份证正反面", "法定代表人身份证扫描"),
        ("委托代理人", "代理人身份证"),
    ),
    # 项目经理资历表后附其证件(建造师证/身份证/职称/社保);找不到该表则走卷尾兜底。
    # 简历表独有"拟在本标段…担任职务",按表头关键词稳命中该表(不在目录里)。
    "项目经理资历表": (
        ("拟在本标段",),
        ("项目经理资历", "项目经理简历", "拟派项目经理", "项目负责人简历", "项目经理身份证"),
        (),
    ),
    # 总工简历表也含"拟在本标段",故用"技术负责人/总工"区分,避免命中项目经理那张。
    "项目总工资历表": (
        ("技术负责人", "项目总工", "总工程师"),
