...
def _enrich_confirmed_outline(
    ...
    ① 光秃秃的"…工程的施工方案"**汇总单节** → **保留该章标题作章级概述引子** +
      按工程量清单拆成工序子节挂其后(占比定详略的深度机制得以生效;已含"·"工序节的
      大纲不动)。章标题必须留:招标施工组织设计的编制要点是带编号的(1~8),评委逐条
      对号,丢了"2、重点、关键和难点工程的施工方案"这条=技术标漏项、编号断裂(埇桥实测);
    ...
    for s in real:
        ...
        if (
            ...
            if disciplines:
                logger.info(
                    "确认大纲的'%s'为单节:保留章标题作概述引子 + 按工程量清单拆成 %d 道工序节",
                    title[:24], len(disciplines),
                )
                # 章标题保留(编号完整,评委按招标编制要点逐条对号),写成章级概述引子;
                # 具体施工工艺分述于后续各工序节,本节不重复展开(否则与工序节内容打架)。
                out.append(
                    BidSectionOutline(
                        title=title,
                        required=True,
                        source_item=getattr(s, "source_item", "") or "",
                        focus_points=[
                            "本章为『重点、关键和难点工程的施工方案』的章级概述:结合工程量清单"
                            "点明本工程的重点、关键、难点工程范围与总体应对思路,并说明各分项"
                            "施工方案分述于后;本节只作纲领性说明,不重复展开后续各工序节的"
                            "具体施工工艺。"
                        ],
                    )
                )
