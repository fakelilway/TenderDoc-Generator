...
def build_bid_outline(
    ...
    # (2) minimal neutral — NEVER impose a detailed default template
    # (招标各不相同，有的目录就很简单). The bidder edits the draft in the outline
    # editor before generation (P1 human gate). 风格案例模板已整体移除(2026-07-03,
    # 从未影响写作、只在此兜底且被 _expand_thin_outline 重建盖过)。
...
def build_bid_document_outline(
    ...
    technical_outline = build_bid_outline(requirements)
    technical_children = [
