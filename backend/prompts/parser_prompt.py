...
- bid_format_requirements 是给生成 Agent 的格式指令，必须从“投标文件的组成”“投标文件格式”“投标文件的编制”“密封和标记”等章节总结：投标文件分几卷/几部分、每部分必须包含哪些表单（投标函、法定代表人身份证明、授权委托书、保证金凭证、各类承诺/声明等，按招标文件原文列全）、正副本份数、签字盖章要求、装订密封要求。用多行文本输出，每行一条，行首用“- ”；找不到相关章节才留空字符串。
