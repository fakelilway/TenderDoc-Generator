...
def build_node_fill_prompt(
    *,
    ...
    for chunk in (knowledge_chunks or [])[:8]:
        ...
        if not content:
            continue
        # "公司同类施工方案"=本节写作的骨架/底稿(沿用结构/工艺/深度,数据换本项目);
        # 其余=一般证据素材。施工方案给更大篇幅,让骨架尽量完整进窗口。
        meta = chunk.get("metadata") or {}
        is_plan = meta.get("document_category") == "施工方案"
        label = (
            "【公司同类施工方案·以此为骨架:沿用结构/工艺/深度,数据换本项目】"
            if is_plan
            else "【素材】"
        )
        snippets.append(f"{label}\n{redact_pii(content)[: 1100 if is_plan else 700]}")
    ...
    if boq_brief:
        ...
        boq_rule = (
            "\n9. 在已满足招标要求(评分点/废标项)的前提下,按上面【BOQ 与造价占比】定详略——"
            "主导分部分项重点详写、工艺最细,正文须引用清单里的具体工程量(数值+单位);"
            "既无对应清单项、又不涉及任何评分点/废标项的纯凑数分部才可一句话带过,"
            "绝不为凑篇幅堆砌无关分部,也绝不因占比低而省略与招标要求相关的内容。"
        )
...
## 知识库参考（含公司同类工程历史施工方案:以其为骨架/底稿沿用结构·工艺·深度,所有数据换成本项目,严禁照搬其具体数值/地名/项目名）
{chr(10).join(snippets) if snippets else '（未匹配到相关知识片段）'}
{guidance_block}
{boq_block}
## 本节点须逐条正面响应的评分点（漏答一条＝对应失分）
{score_block or '（本节点无直接对应评分点，按通用施工组织设计深度撰写）'}

## 本节点绝不可触碰的废标红线（命中任一条＝整体废标）
...
## 写作规则
0. 【最高准则·压过本节其它一切要求】本节写作的第一目标是满足"这份招标文件"的硬要求。
   优先级自上而下:① 逐条正面响应上面的评分点、绝不触碰废标红线 ＞ ② 按 BOQ 占比定详略 ＞
   ③ 以知识库施工方案为骨架。三者冲突时一律以招标要求为准:可以偏离历史方案的结构、可以
   打破 BOQ 的篇幅分配,也必须把每条评分点写到、把每条废标红线避开。任何与评分点/废标项相关
   的内容,即使所在分部 BOQ 占比低或无对应清单项,也绝不能用"本工程不涉及/一句话带过"省略。
1. 只写本节点正文，不得输出任何标题或 Markdown 标题符号。
2. 每节至少 8 段连贯论述，包含工程参数、操作步骤、验收标准和应急预案。
   操作步骤须**按施工工序的先后逻辑顺序**排列(如路基:测量放样→清表→处理→分层填筑→压实检测;
   不得颠倒工序),让评审看出施工组织的合理性。
3. 列表型内容可用 Markdown 表格，表格应有实际字段和值；无依据的值留"________"。
4. 不写"人工确认点""待补充""TODO""AI生成"等元话语。
5. 不编造金额、人名、证号、日期；不得改变招标文件结构。
   在不与招标要求(评分点/废标项)冲突的前提下,标【公司同类施工方案】的片段作为本节写作的
   **骨架/底稿·专业深度参照**:沿用它的章节结构、工艺步骤顺序、措施条理与详细深度,以此打底
   再充实展开,使本节达到与之相当的专业深度;但所有工程参数(项目名、地点、规模、工期、工程量、
   数据)**一律替换为本项目的真实数据**——以招标文件与上面【BOQ 与造价占比】的工程量为准,
   **严禁照搬历史项目的具体数值/地名/项目名/人名**;不得因贴骨架而省略或弱化任何评分点/废标响应。
6. **逐条正面响应**上面"须逐条正面响应的评分点"——每条都要有对应的、可量化的工程措施与
   验收标准,点名响应、不要泛泛而谈;漏答一条即对应失分。
7. **逐条规避**上面"绝不可触碰的废标红线"——涉及承诺(工期/质量/安全/资格等)用确定性表述
   明确承诺并满足招标限值;命中任一条即整体废标,务必正面、实质地规避而非一笔带过。{length_rule}{boq_rule}
