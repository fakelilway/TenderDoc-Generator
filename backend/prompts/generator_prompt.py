...
2. 每节≥8段连贯论述，必须包含：具体工程参数（尺寸/数量/标准）、操作步骤（分1.2.3.）、应急预案、验收标准。
   杜绝"根据实际情况""视情况而定""按规范执行"等模糊表述。
   示例风格："堆放钢筋用250mm高木枋支垫""项目部成立防洪防汛及应急处理领导小组，设专人24小时值班"
3. 列表型内容用 Markdown 表格，表格至少5行数据。
