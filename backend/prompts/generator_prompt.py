...
# Textual summary of how a real, submittable Chinese bid reads and is laid out.
# This is injected into every generation prompt so the model writes production
# prose instead of meta-annotated drafts.
REAL_BID_FORMAT_SPEC = """真实投标文件格式与文风规范（务必逐条遵守）：
1. 文风：通篇正式、书面、承诺性工程语言，主语用“我单位／本公司／本项目部”。禁止口语、解释性旁白、对话语气以及“以下是／作为AI／本文档”等元话语。
2. 标题层级：技术标施工组织设计使用真实标书层级“第X章、 → 第X节、 → 一、 → （一） → 1. → （1）”；商务标使用“一、 → （一） → 1.”。标题简洁，不带说明性后缀。
...
你的任务：基于 parser agent 抽取的招标文件关键信息、检索到的企业真实投标文件/企业自有素材、以及真实投标文件模板结构，生成一份可直接套用排版、继续补充真实数据后递交的正式投标文件正文。输出必须同时覆盖【技术标】和【商务标】，且技术标必须排在商务标之前。

{REAL_BID_FORMAT_SPEC}
...
5. 标题编号使用“一、1.（1）①”四级风格；施工组织设计章节优先使用“第X章、第X节、一、（一）”的真实标书层级。
6. 真实模板优先级高于通用写法：如果模板给出了主目录、施工组织设计目录、附表或固定表单，必须优先沿用模板顺序和章节名称。
"""
