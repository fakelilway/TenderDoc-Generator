...
3. 投标文件卷册和章节顺序必须以 BidTemplate 主目录为准；若招标文件采用第一信封/第二信封或资格标/技术标/商务标拆分，必须沿用对应顺序，不得强行改成技术标先行。
4. 完整标书必须覆盖资格/商务固定表单、技术标/施工组织设计、附表和报价/经济标说明；报价只生成目录和编制说明，具体数值留空白由投标人填写。
...
def build_document_prompt(
    ...
    user_prompt = f"""请根据招标文件解析结果和企业真实投标文件模板，生成一份完整的“投标文件正文”。必须覆盖资格/商务文件、技术文件/施工组织设计、附表以及报价/经济标说明；卷册和章节顺序以 BidTemplate 主目录为准，不得强行改成技术标先行。
...
输出规则：
- 第一行使用 `# {requirements.project_name or '投标项目'} 投标文件`。
- 标题下方写 `投标人：{company_name}`，随后写一段“投标文件响应总说明”正文（不加任何确认点）。
- 必须按 BidTemplate 主目录输出；若模板主目录先列投标函、授权委托、保证金、资格资料，再列施工组织设计，就按该顺序生成。
- 技术标/施工组织设计正文必须按【确认生成结构】输出；如果模板提供附表，必须生成对应附表标题与正式的表格/用途说明。
- 商务/资格章节必须优先沿用 BidTemplate 中的固定表单、资格、声明章节；若模板没有给出商务表单，才使用投标函、授权委托、资格审查、投标保证金、承诺函作为兜底。
- 报价/经济标若模板没有提供完整卷册，正文采用正式“报价文件目录/报价编制说明”口吻，金额、清单和综合单价保留下划线空白，不得编造；严禁出现系统能力说明、编制人指令或后台流程说明。
- 项目管理机构、资格响应、报价目录、进度计划、劳动力计划、机械设备计划、附表必须优先使用 Markdown 表格；导出 DOCX 时系统会转成真实 Word 表格。
- 若【可插入知识库图片资料】中存在与资格/人员/业绩/图纸匹配的图片资料，可在相应章节单独一行插入 `{{knowledge_image:document_id=数字 caption="图片说明"}}`。只能使用清单中给出的 document_id。
