...
def test_update_template_renames_and_tags(monkeypatch) -> None:
    ...
    summary = template_service.update_template(
        1, name="公路模板(已改名)", tags=["公路", "高速"]
    )
...
def test_recommend_templates_ranks_matching_type_first(monkeypatch) -> None:
    cursor = FakeCursor(
        [
            _template_row(id=2, name="房建模板", project_type="房屋建筑", specialty="建筑", tags=["房建"]),
            _template_row(id=1, name="公路模板", project_type="公路工程", specialty="道路", tags=["公路"]),
        ]
    )
    monkeypatch.setattr(template_service, "_connect", lambda: FakeConnection(cursor))
