...
def _template_row(**overrides):
    profile_json = {
        ...
        "tone_rules": [],
        "forbidden_phrases": [],
        "generated_by": "deterministic_template_profile_agent",
        "notes": [],
    }
    row = {
        "id": 1,
        ...
        "template_profile_json": profile_json,
        "created_by": 1,
        "created_at": None,
    }
...
def test_create_template_parses_and_inserts(monkeypatch) -> None:
    ...
    assert summary["profile_generated_by"] == "deterministic_template_profile_agent"
    insert_statement, _params = cursor.statements[0]
    assert "INSERT INTO bid_templates" in insert_statement
    assert "template_profile_json" in insert_statement
...
def test_seed_template_from_json_inserts_when_missing(monkeypatch, tmp_path) -> None:
    ...
    assert params[9].adapted["template_name"] == "默认公路第一信封模板"
...
def test_seed_template_from_json_backfills_existing_profile(
    monkeypatch, tmp_path
) -> None:
    template_path = tmp_path / "default_template.json"
    template_path.write_text(
    ...
    assert "UPDATE bid_templates" in cursor.statements[1][0]
...
def test_update_template_renames_and_tags(monkeypatch) -> None:
    ...
    update_statement, params = cursor.statements[1]
    assert "template_profile_json" in update_statement
    assert params[7].adapted["generated_by"] == "deterministic_template_profile_agent"
...
def test_template_profile_for_project_returns_stored_profile(monkeypatch) -> None:
    cursor = FakeCursor(
        [
            {
                "template_profile_json": _template_row()["template_profile_json"],
                "template_json": None,
                "project_type": "公路工程",
                "specialty": "道路",
            }
    ...
    profile = template_service.template_profile_for_project(7)

    assert profile is not None
    assert profile.generated_by == "deterministic_template_profile_agent"


def test_template_profile_for_project_builds_fallback_from_template(
    monkeypatch,
) -> None:
    template_json = BidTemplate(
        template_name="公路模板",
