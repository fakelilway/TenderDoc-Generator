...
另有确定性客观打分器 `backend/services/docx_health_check.py`，对落盘 `.docx` 直接算 0-100 质量分（6 项加权：必填字段/表格填充/章节齐全/残留物/字体一致/篇幅响应）：

- `score_docx(单卷)` —— 单个 `.docx` 的质量分；
- `score_delivery(按卷)` —— 按卷拆分母：必填/表格量用商务卷、章节/篇幅量用技术卷、残留两卷取严、字体仅技术卷。

出标（`generation_service.export_markdown_for_project`）后还挂了非阻断钩子 `backend/services/delivery_quality.py`（`score_delivery_files` / `score_project_delivery`），自动按卷打分并落 `backend/eval_results/project_{id}.json`（已 gitignore）；打分失败绝不影响出标。

## 基线现状（2026-06-16 实测）

| 文件 | 页数 | 字数(压缩) | 备注 |
|------|------|-----------|------|
| 基线#1 真实投标文件 | 183 | 69,877 | 全卷含附件，作对标 |
| 当前生成稿（项目114 重生成） | — | 79,722 | 27 章，技术卷全长 |
| 当前生成稿（项目116/117） | 141 | 76,000–78,000 | 两卷全长 |

> 现行管线已能产出**全长**标书：技术卷字数约对标基线 **100%+**、页数约 **77%**。早先「生成稿仅 7K 字、低一个数量级、篇幅是首要短板」的判断已过时，篇幅与深度不再是首要短板。

## 阈值（满意线）

| 维度 | 指标 | 阈值 | 当前 | 状态 |
|------|------|------|------|------|
| 篇幅 | 技术卷字数 / 基线技术卷字数 | ≥ 60% | ~100%+ | ✅ |
| 篇幅 | 技术卷页数 | ≥ 基线技术卷 70% | ~77% | ✅ |
...
> 注：下文数字为当时的小稿（技术正文 6169 字）。现行管线已产出全长标书（见上「基线现状」），但本段关于**响应率分母错位**的论述至今仍成立，保留。
...
> **重要：响应率必须按卷分母。** 技术评分项 vs 技术正文；商务/报价废标 vs 商务卷照抄+填空 + reviewer 审查。拿全部 27 个（多为商务/报价）废标比技术正文 → 7% 是分母错位，不是质量问题。**「按卷拆分母」已在 `docx_health_check.score_delivery` 落地**（必填/表格量商务卷、章节/篇幅量技术卷、残留两卷取严、字体仅技术卷）。
...
- `benchmark_vs_baseline.py` 的响应率指标需要先用 Parser 解析出 `requirements.json`（含 `technical_score_items`/`invalid_bid_items`）才能算。（按卷拆分母已在 `docx_health_check.score_delivery` 落地。）
