...
- `backend/scripts/prepare_knowledge_manifest.py`：批量扫描资料目录，生成规范命名/标签 manifest，可按 manifest 导入知识库。
...
| Generator Agent | 已实现 | 默认长上下文一次生成三卷 Markdown；分章节生成保留为 fallback |
...
| Workflow 编排 | service 状态 | Redis/DB 持久化 workflow state，生成内核可切换 |
...
5. `Generator Agent`：默认使用长上下文单次生成，把确认目录、招标要求、BidPlan、精选资料和图片候选一次性交给模型；旧 section-by-section 生成只作为 fallback。
...
| 批量入库 | Manifest CSV/JSON + 现有入库 service | 默认 dry-run，显式 `--import-to-kb` 才写 DB/MinIO |
...
当前默认生成模式下，RAG/知识库不再承担“分章节拼正文”的结构职责，而是作为资料治理和选择层：用户勾选或系统检索出的资料会被打包进长上下文 prompt，图片资料通过 `{{knowledge_image:document_id=...}}` 标记交给 DOCX exporter 真正插入。

批量资料治理采用“原始资料只读保留 -> manifest 审核 -> 复制规范文件 -> 导入知识库”的流程。脚本默认只生成建议清单；导入时复用 `knowledge_service.index_uploaded_knowledge()`，不会另走旁路写库。
