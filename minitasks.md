...
| M9 | ✅ | 前端 UX 重构 | 移除大纲编辑，Tab 化中心列，渐进展示，进度浮层可关闭 |

## 当前进行中

| 编号 | 状态 | 内容 | 验收标准 |
|------|------|------|----------|
| M10 | 🔧 | 技术正文质量提升 | 施工组织设计有工程针对性、足够篇幅、响应评分项 |
| M11 | 🔧 | 真实中标标书基线 | 用真实中标标书对比页数、表格密度、章节深度、填空率 |
| M12 | 🔧 | 知识库真实资料入库 | 公司证件、人员证件、业绩、技术方案、图片资料完成命名和标签 |
| M13 | 🔧 | 样本项目端到端验收 | 至少 3 个脱敏真实项目跑通上传、生成、审查、下载 |

## 待做

| 编号 | 优先级 | 内容 | 说明 |
|------|--------|------|------|
| M14 | P0 | 真实格式回归集 | 收集多份招标文件及对应格式页，测试目录、函件、表格、签章位是否原样 |
| M15 | P0 | DOCX 视觉回归 | 导出后自动渲染关键页，对比是否出现裁切、重叠、空白页、错卷 |
| M16 | P0 | 知识库 OCR 和 `.doc` 兼容 | 支持公司资料里的扫描件、JPG/PNG、老 Word 文档入库 |
| M17 | P1 | 内网部署包 | Docker Compose 单机版、Nginx、HTTPS、环境变量模板 |
| M18 | P1 | 备份恢复和审计 | PostgreSQL/MinIO 备份、恢复演练、上传下载删除审计日志 |
| M19 | P1 | 长任务队列 | 解析、生成、导出从 BackgroundTasks 迁移到可重试队列 |
| M20 | P1 | 新点软件交付实测 | 用导出的 DOCX 在新点投标文件制作软件中导入并记录损失项 |
...
- `frontend/components/ParsedReviewPanel.tsx`：前端展示解析结果供人工确认。
...
4. 修正解析错误后确认（系统自动生成大纲，确认后即可开始生成）。
