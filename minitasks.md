...
本文件包含从零开始构建 MVP 和下一阶段产品化 workflow 的完整任务清单，共 **60 个 Minitasks**（M1–M60）。
每个任务包含：**完成状态**、**依赖**、**完成标准**、**测试方法**。M1–M44 已形成可演示 MVP；M45–M60 用于把 MVP 升级为更接近真实标书工作流的产品。
...
- 共 **60 个 Minitasks**，从 M1 到 M44 覆盖 MVP，M45 到 M60 覆盖下一阶段 workflow 产品化。
- 下一阶段优先顺序：M45–M48 先补人工可控和真实状态，再做 M49–M52 的知识库引用/在线编辑/版本，最后做 M53–M60 的策略 Agent 和输出体验。
