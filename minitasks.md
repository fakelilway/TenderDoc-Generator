...
| 5. 前端工作台 | M33–M40 | 登录/注册/RBAC、项目工作台、知识库/模板库/账号管理页面 |
...
| 10. 真实模板学习 | M61–M64 | 真实投标 PDF → 脱敏 BidTemplate JSON、模板库管理与推荐 |
| 11. 正奇专用化 | M65–M67 | 产品范围收敛、知识库结构化标签、默认公路模板 |
...
- 默认生成内核切换为**长上下文单次生成**（`BID_GENERATION_MODE=long_context`）：
  确认目录、招标要求、模板约束、招标全文、企业档案、精选资料和图片清单一次性交给
...
- **目标形态**：
  ...
  - 模板库降级：退为"技术标深度参考 + 招标文件未规定时的兜底骨架"，不再是权威。
