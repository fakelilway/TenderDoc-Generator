...
| M10 | ✅ | 技术正文深度大纲 | 招标技术大纲薄时扩展为 25 节标准施工组织设计大纲（对标真实中标标书设计+对抗式审查），逐节注入评分项/废标项/必覆盖要点/字数预算+不达标重写。**E2E 实测(招标#1,空知识库)：技术正文 6K→76,875 字(≈13×,超中标标书全卷)、25 节、评分点 3/3、审查通过、内容真材实料(C30面层180mm/级配/压实度等工程参数+项目针对性)** |
...
| 编号 | 状态 | 内容 | 验收标准 |
|------|------|------|----------|
| M13 | ✅ | 样本项目端到端验收（招标#1） | 招标#1–#4 格式跑通（#4 DOCX 定位 bug 已修）；技术正文 E2E 实测 76,875 字/25 节/评分点 3/3/审查通过。剩 #2–#4 完整端到端（含 LLM 正文）可后续抽测 |
| M12 | 🔧 | 知识库真实资料入库 | 现有 14 个资料(证件扫描图×13 + 业绩表×1)，但**只有标签无 OCR 文本、无技术素材、公司档案关键字段空**。需：①填公司档案(地址/法代/资质/安许号/电话) ②OCR 读证件(见 M16) ③补技术文本素材 ④标签去重 |
| M16 | 🔧 | 知识库 OCR | 证件扫描件(JPG/PNG)文字提取，让证号/有效期/资质等级可检索可填空 |
...
| 编号 | 优先级 | 内容 | 为什么是下一步 |
|------|--------|------|----------------|
| **M12/M16** | **P0** | **知识库真实资料 + OCR**（进行中） | 现有 14 份证件/业绩，但只有标签无 OCR 文本、无技术素材、公司档案空。①填公司档案立刻让商务字段填全 ②OCR 读证件 ③补技术文本素材。**这是质量提升的下一个真正瓶颈。** |
