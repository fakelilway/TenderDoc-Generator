...
## 当前主线：格式优先生成内核（新 M1–M10）

> 旧 M1–M84 是历史开发编号，保留在下方作归档。自 2026-06-13 起，生成质量主线改为下面这套新 M1–M10：先保证招标文件格式绝对正确，再逐步提高填充质量、DOCX 版式和生产化能力。

| 阶段 | 状态 | 任务 | 完成标准 |
|------|------|------|----------|
| M1 | ✅ 已完成 | 确定性格式骨架 | 从 `format_outline_tree` 生成商务/技术/报价三卷 Markdown 骨架；跳过“投标文件（商务文件）”这类根容器；缺完整三卷格式树时生成直接失败 |
| M2 | ✅ 已完成 | 原文模板抽取 | 从招标文件格式章节抽取函件、表格、清单说明原文并嵌入骨架；双信封同名投标函按卷抽取；目录行不抢模板；泛标题只作边界 |
| M3 | ⬜ 待做 | 节点级填槽 | Generator 不再让 LLM 输出整卷 Markdown，而是对每个叶子节点返回填充内容；失败可按节点重试；标题和表头由代码保留 |
| M4 | ⬜ 待做 | 表格/字段结构化 | 把表单字段、表头、行列、签字盖章位解析为结构化对象；可审计“列是否缺失、字段是否空、是否乱改表头” |
| M5 | ⬜ 待做 | 人工格式确认台 | 前端在生成前展示三卷骨架、原文模板、缺失模板和风险提示，制作员可确认/修正后再生成 |
| M6 | ⬜ 待做 | DOCX 版式映射 | 将结构化表单/表格映射到 Word 表格、标题样式、页边距、分册和页眉页脚；减少“Markdown 看着对、Word 表格乱”的问题 |
| M7 | ⬜ 待做 | 知识库证据绑定 | 将公司证件、人员证件、业绩、图片、技术图表绑定到具体格式节点，而不是只作为普通 RAG 文本 |
| M8 | ⬜ 待做 | 合规硬审计 | 用代码审标题、表头、必填字段、签字盖章位和图片/附件缺失；LLM 只审内容风险和逻辑矛盾 |
| M9 | ⬜ 待做 | 真实样本评测 | 至少 3 个脱敏真实项目端到端评测，输出目录完整率、表格一致率、人工修改量、格式问题数、废标风险检出率 |
| M10 | ⬜ 待做 | 公司试用交付 | 内网部署、备份审计、长任务队列、新点软件导入边界测试；制作员可完成真实试用闭环 |
