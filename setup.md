...
> 📦 **要在公司主机上正式部署到局域网、供多台电脑用浏览器访问？请看 [`docs/局域网部署指南.md`](docs/局域网部署指南.md)（Docker 一键版，数据全部存本机）。**
>
> 本文是**开发者在本机（localhost）运行调试**的指南。

本文说明如何在本机运行开发/调试版本，已包含前端、后端、数据库、对象存储、知识库、公司档案、生成工作流和 DOCX 导出。**正式投入使用（公司内网服务器）请按上面的局域网部署指南走，而非本文的 localhost 流程。**
...
7. 查看并确认解析结果，重点核对项目名、招标人、工期、质量、资质、评分项、废标项和格式目录树。确认后系统自动生成大纲并进入可生成状态。
8. 选择资料，然后点击生成。后端会照抄招标格式章为商务卷并把已知字段烧录进表单，按标准施工组织设计大纲写技术正文，再审查。
9. 查看实时状态和失败原因。失败时修正配置、资料或解析结果后重试。
10. 查看预览、审查报告、响应矩阵、评分预测。
11. 在线编辑正文，保存后重新审查。
12. 终审确认后下载商务卷 + 技术卷 DOCX、Markdown 或审查报告（报价卷由外部造价软件单独做）。
